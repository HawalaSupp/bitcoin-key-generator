...
enum EdgeCaseGuards {
    ...
    // MARK: - #56: Spam Asset Label Filter
    
    /// Basic heuristic to detect spam/scam asset labels.
    static func isLikelySpamAssetLabel(name: String, description: String = "") -> Bool {
    ...
    // MARK: - #57: Asset Metadata Fallback
    
    /// Returns a safe fallback name when token metadata fails to load.
    static func assetFallbackName(tokenId: String, contractAddress: String) -> String {
        let shortContract = contractAddress.prefix(6) + "…" + contractAddress.suffix(4)
        return "Asset #\(tokenId) (\(shortContract))"
    }
