...
private struct SessionRequestSheet: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let requestedSignerAddress = requestedSignerAddress {
                    DetailItem(label: "Signer", value: requestedSignerAddress)
                }

                if let signingMethodMessage = signingMethodMessage {
                    HStack(alignment: .top, spacing: 10) {
                        Image(systemName: "key.horizontal.fill")
                            .foregroundStyle(.orange)

                        Text(signingMethodMessage)
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    .padding(12)
    ...
    private var canApprove: Bool {
        if decodedTx?.riskLevel == .critical {
            return false
        }
        ...
        case "personal_sign", "eth_sign", "eth_signTypedData", "eth_signTypedData_v3", "eth_signTypedData_v4":
            guard let requestedSignerAddress = requestedSignerAddress else {
                return false
            }
            return normalizedAvailableSignerAddresses.contains(normalizeEVMAddress(requestedSignerAddress))
        default:
            return true
        }
    }

    private var requestedSignerAddress: String? {
        switch request.method {
        case "eth_sign":
            guard let params = request.params as? [Any],
                  params.count >= 2,
        ...
        case "personal_sign":
            ...
            if let first = params[0] as? String, looksLikeEVMAddress(first) {
                return first
            }
            if let second = params[1] as? String, looksLikeEVMAddress(second) {
                return second
            }
            return nil
        case "eth_signTypedData", "eth_signTypedData_v3", "eth_signTypedData_v4":
    ...
    private var normalizedAvailableSignerAddresses: Set<String> {
        Set(availableAccounts.compactMap(extractAddress(fromCAIP10:)).map(normalizeEVMAddress))
    }

    private var signingMethodMessage: String? {
        switch request.method {
        case "personal_sign", "eth_sign", "eth_signTypedData", "eth_signTypedData_v3", "eth_signTypedData_v4":
            if let requestedSignerAddress = requestedSignerAddress,
               !normalizedAvailableSignerAddresses.contains(normalizeEVMAddress(requestedSignerAddress)) {
                return "This request targets \(requestedSignerAddress), which is not available in the active WalletConnect software account set. Hawala will reject it instead of signing with a different key."
            }
            return "This approval will be signed by Hawala's current software wallet key. Hardware-wallet signing is not available for this WalletConnect request type yet."
        case "eth_sendTransaction", "eth_signTransaction":
            return "WalletConnect transaction requests are not signed here. Review and sign the transaction through Hawala's send flow instead."
        default:
            return nil
        }
    }

    private func extractAddress(fromCAIP10 value: String) -> String? {
