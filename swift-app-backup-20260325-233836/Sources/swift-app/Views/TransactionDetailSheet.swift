    ...
    private var detailsCard: some View {
        VStack(spacing: 0) {
            ...
            if let counterparty = transaction.counterparty, !counterparty.isEmpty {
                ...
                TxDetailRow(
                    label: transaction.type == "Send" ? "Recipient" : (transaction.type == "Bridge" ? "Route" : "Sender"),
                    value: counterparty,
                    isMonospace: true,
                    canCopy: true,
            ...
            if let destinationTxHash = transaction.secondaryTxHash {
                ...
                TxDetailRow(
                    ...
                    isMonospace: true,
                    canCopy: true,
                    onCopy: { copyToClipboard(destinationTxHash) }
                )
            }
            ...
            if let destinationURL = transaction.secondaryExplorerURL {
                Button {
                    ...
                    HStack {
                        ...
                        Text("View Destination on \(secondaryExplorerName)")
                        Spacer()
                        Image(systemName: "arrow.up.right.square")
                            .font(.subheadline)
                    }
