...
struct TransactionHistoryRow: View {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            if let route = entry.counterparty, entry.type == "Bridge" {
                                HStack(spacing: 4) {
                                    Text("Route:")
                                        .font(.caption2)
                                        .foregroundStyle(.secondary)
                            ...
                            if let destinationHash = entry.secondaryTxHash {
                                HStack(spacing: 4) {
                                    Text("Destination TX:")
                                        .font(.caption2)
                                        .foregroundStyle(.secondary)
                                    Text(String(destinationHash.prefix(16)) + "..." + String(destinationHash.suffix(8)))
                                        .font(.caption2.monospaced())
                                        .foregroundStyle(.primary)
                                }
                            }

                            if let note = entry.detailNote, !note.isEmpty {
                                Text(note)
                                    .font(.caption2)
                                    .foregroundStyle(.secondary)
                        ...
                        if let url = entry.secondaryExplorerURL {
                            Button {
                                #if canImport(AppKit)
                                NSWorkspace.shared.open(url)
                                #elseif canImport(UIKit)
                                ...
                                Label("Destination Explorer", systemImage: "point.3.connected.trianglepath.dotted")
                                    .font(.caption)
                            }
                            .buttonStyle(.bordered)
                            .controlSize(.small)
    ...
    private var hasDetails: Bool {
        entry.txHash != nil || entry.fee != nil || entry.blockNumber != nil || entry.providerName != nil || entry.secondaryTxHash != nil || entry.detailNote != nil
    }
    ...
    private var statusColor: Color {
        ...
        case "processing", "arriving", "in transit": return .blue
