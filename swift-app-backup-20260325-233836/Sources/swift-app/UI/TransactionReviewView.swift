...
struct TransactionReviewView: View {
    ...
    @State private var acknowledgedMissingDestinationTag = false
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    if transaction.requiresDestinationTagAcknowledgement {
                        missingDestinationTagWarning
                    }
    ...
    private var recipientSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                if let contact = ContactsManager.shared.contact(forAddress: transaction.recipientAddress, chainId: transaction.chainId) {
                    HStack(spacing: 6) {
                ...
                if let destinationTag = transaction.destinationTag {
                    HStack(spacing: 6) {
    ...
    private var missingDestinationTagWarning: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("No destination tag provided")
                        .font(.subheadline.weight(.semibold))
                    Text("Only continue if the recipient does not require an XRP destination tag.")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
            ...
            Toggle(isOn: $acknowledgedMissingDestinationTag) {
                Text("I confirmed this recipient does not require a destination tag")
                    .font(.caption.weight(.medium))
            }
    ...
    private var actionButtons: some View {
        VStack(spacing: 12) {
            ...
            .disabled(isConfirming || transaction.hasInsufficientBalance || (transaction.requiresDestinationTagAcknowledgement && !acknowledgedMissingDestinationTag))
...
struct TransactionReviewData {
    ...
    var isXRP: Bool {
        ContactsManager.chainIDsMatch(chainId, "xrp-mainnet") || ContactsManager.chainIDsMatch(chainId, "xrp-testnet")
    }

    var requiresDestinationTagAcknowledgement: Bool {
        isXRP && (destinationTag?.isEmpty ?? true)
    }
