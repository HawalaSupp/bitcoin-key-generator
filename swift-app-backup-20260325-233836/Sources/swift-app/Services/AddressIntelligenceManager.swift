...
final class AddressIntelligenceManager: ObservableObject {
    ...
    private var sentHistory: [String: SentHistoryEntry] = [:]
    ...
    func analyzeAddress(_ address: String) async -> AddressAnalysis {
        ...
        let sentHistoryEntry = sentHistoryEntries(for: normalizedAddress)
            .max { lhs, rhs in lhs.lastDate < rhs.lastDate }
    ...
    func quickRiskCheck(_ address: String) -> AddressRiskLevel {
        ...
        if !sentHistoryEntries(for: normalized).isEmpty {
            return .safe
        }
    ...
    /// Quick check for an address on a specific chain.
    func quickRiskCheck(_ address: String, chainId: String) -> AddressRiskLevel {
        let normalized = normalizeAddress(address, blockchain: AddressBlockchain.detect(from: address)).lowercased()
        ...
        if sentHistory[sentHistoryKey(for: normalized, chainId: chainId)] != nil {
            return .safe
        }
    ...
    func recordSend(to address: String) {
        let blockchain = AddressBlockchain.detect(from: address)
        let normalized = normalizeAddress(address, blockchain: blockchain)
        recordSend(to: normalized, chainId: canonicalHistoryChainId(for: blockchain))
    }

    /// Record a send to an address on a specific chain.
    func recordSend(to address: String, chainId: String) {
        let blockchain = AddressBlockchain.detect(from: address)
        let normalized = normalizeAddress(address, blockchain: blockchain)
        let key = sentHistoryKey(for: normalized, chainId: chainId)

        if var existing = sentHistory[key] {
            existing.count += 1
            existing.lastDate = Date()
            sentHistory[key] = existing
        } else {
            sentHistory[key] = SentHistoryEntry(address: normalized, chainId: chainId.lowercased(), count: 1, lastDate: Date())
        }
        
        // Update label if exists
        if var label = addressLabels[normalized.lowercased()] {
            label.lastUsed = Date()
            label.sendCount += 1
            addressLabels[normalized.lowercased()] = IntelAddressLabel(
                id: label.id,
    ...
    func isFirstTimeSend(to address: String) -> Bool {
        let blockchain = AddressBlockchain.detect(from: address)
        let normalized = normalizeAddress(address, blockchain: blockchain)
        return sentHistoryEntries(for: normalized).isEmpty
    }

    /// Check if this is first time sending to the address on the current chain.
    func isFirstTimeSend(to address: String, chainId: String) -> Bool {
        let blockchain = AddressBlockchain.detect(from: address)
        let normalized = normalizeAddress(address, blockchain: blockchain)
        return sentHistory[sentHistoryKey(for: normalized, chainId: chainId)] == nil
    }
    
    /// Returns the most recently sent-to addresses, sorted by lastDate descending (ROADMAP-05 E7)
    func getRecentRecipients(limit: Int = 5) -> [(address: String, chainId: String, count: Int, lastDate: Date)] {
        getRecentRecipients(limit: limit, chainId: nil)
    }

    /// Returns recent recipients, optionally filtered to a specific chain.
    func getRecentRecipients(limit: Int = 5, chainId: String?) -> [(address: String, chainId: String, count: Int, lastDate: Date)] {
        sentHistory.values
            .filter { entry in
                guard let chainId else { return true }
                return ContactsManager.chainIDsMatch(entry.chainId ?? chainId, chainId)
            }
            .sorted { $0.lastDate > $1.lastDate }
            .prefix(limit)
            .map {
                (address: $0.address, chainId: $0.chainId ?? canonicalHistoryChainId(for: AddressBlockchain.detect(from: $0.address)), count: $0.count, lastDate: $0.lastDate)
            }
    }
    ...
    private func loadSentHistory() {
        ...
        if let data = userDefaults.data(forKey: key),
           let decoded = try? JSONDecoder().decode([String: SentHistoryEntry].self, from: data) {
            var migrated: [String: SentHistoryEntry] = [:]
            for (storedKey, entry) in decoded {
                let rawAddress = entry.address.isEmpty ? storedKey : entry.address
                let normalizedAddress = normalizeAddress(rawAddress, blockchain: AddressBlockchain.detect(from: rawAddress))
                let normalizedChainId = (entry.chainId ?? canonicalHistoryChainId(for: AddressBlockchain.detect(from: normalizedAddress))).lowercased()
                migrated[sentHistoryKey(for: normalizedAddress, chainId: normalizedChainId)] = SentHistoryEntry(
                    address: normalizedAddress,
    ...
    private func sentHistoryKey(for address: String, chainId: String) -> String {
        let normalizedAddress = normalizeAddress(address, blockchain: AddressBlockchain.detect(from: address)).lowercased()
        return "\(chainId.lowercased())|\(normalizedAddress)"
    }

    private func sentHistoryEntries(for address: String) -> [SentHistoryEntry] {
        let normalizedAddress = normalizeAddress(address, blockchain: AddressBlockchain.detect(from: address)).lowercased()
        return sentHistory.values.filter { $0.address.lowercased() == normalizedAddress }
    }

    private func canonicalHistoryChainId(for blockchain: AddressBlockchain) -> String {
        switch blockchain {
        case .bitcoin:
