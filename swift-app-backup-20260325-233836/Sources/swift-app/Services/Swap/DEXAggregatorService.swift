...
final class DEXAggregatorService: ObservableObject {
    ...
    func checkApproval(
        ...
        guard !wallet.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            throw DEXAggregatorError.approvalFailed("Wallet address missing for approval check")
        }

        if isNativeAsset(token, on: chain) {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                do {
                    ...
                    #if DEBUG
                    print("[DEX] Rust FFI quote retrieval failed: \(error)")
                    #endif
                    continuation.resume(throwing: DEXAggregatorError.quoteBackendUnavailable(
                        "Live swap quotes are unavailable. Hawala no longer falls back to mock DEX quotes. Check Rust routing, provider connectivity, and API configuration."
                    ))
                }
            }
        }
    ...
    private func isNativeAsset(_ token: String, on chain: SupportedChain) -> Bool {
        let normalizedToken = token.lowercased()
        let normalizedNative = chain.nativeToken.lowercased()

        if normalizedToken == normalizedNative {
            return true
        }

        let nativeAliases: Set<String> = [
            "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",
            "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"
        ]

        if nativeAliases.contains(normalizedToken) {
            return chain != .polygon
        }

        if chain == .polygon && normalizedToken == "0x0000000000000000000000000000000000001010" {
            return true
        }
