...
final class WalletConnectSigningService: ObservableObject {
    ...
    func handleSign(_ request: WCSessionRequest, keys: AllKeys) async throws -> String {
        try ensureSoftwareSigningSupported(for: request, keys: keys)
    ...
    private func signPersonalMessage(_ request: WCSessionRequest, keys: AllKeys) async throws -> String {
        let (_, message) = try extractPersonalSignPayload(from: request)
    ...
    private func signTypedData(_ request: WCSessionRequest, keys: AllKeys) async throws -> String {
        let typedDataJSON = try extractTypedDataJSON(from: request)
    ...
    private func ensureSoftwareSigningSupported(for request: WCSessionRequest, keys: AllKeys) throws {
        switch request.method {
        case "personal_sign", "eth_sign", "eth_signTypedData", "eth_signTypedData_v3", "eth_signTypedData_v4":
            guard let requestedAddress = requestedAddress(for: request) else {
                throw WCError.invalidRequest("Missing signer address in WalletConnect request")
            }

            let normalizedRequestedAddress = normalizeEVMAddress(requestedAddress)
            let availableSoftwareAddresses = Set(softwareSigningAddresses(from: keys).map(normalizeEVMAddress))

            if availableSoftwareAddresses.contains(normalizedRequestedAddress) {
                return
            }

            let hardwareAddresses = Set(HardwareWalletManagerV2.shared.savedAccounts.map { normalizeEVMAddress($0.address) })
            if hardwareAddresses.contains(normalizedRequestedAddress) {
                throw WCError.hardwareWalletSigningUnavailable(requestedAddress)
            }

            throw WCError.accountNotAvailable(requestedAddress)

        case "eth_sendTransaction", "eth_signTransaction":
            throw WCError.requestRequiresSendFlow
    ...
    private func requestedAddress(for request: WCSessionRequest) -> String? {
        switch request.method {
        case "personal_sign", "eth_sign":
            return try? extractPersonalSignPayload(from: request).address
        case "eth_signTypedData", "eth_signTypedData_v3", "eth_signTypedData_v4":
            return try? extractTypedDataPayload(from: request).address
        default:
            return nil
        }
    }

    private func softwareSigningAddresses(from keys: AllKeys) -> [String] {
        [
            keys.ethereum.address,
            keys.ethereumSepolia.address,
            keys.bnb.address
        ].filter { !$0.isEmpty }
    }

    private func extractPersonalSignPayload(from request: WCSessionRequest) throws -> (address: String, message: String) {
        guard let params = request.params as? [Any], params.count >= 2 else {
            throw WCError.requestTimeout
        }

        switch request.method {
        case "eth_sign":
            guard let address = params[0] as? String,
                  let message = params[1] as? String else {
                throw WCError.invalidRequest("Invalid eth_sign parameters")
            }
            return (address, message)

        case "personal_sign":
            if let first = params[0] as? String,
               let second = params[1] as? String {
                if looksLikeEVMAddress(first) {
                    return (first, second)
                }
                if looksLikeEVMAddress(second) {
                    return (second, first)
                }
            }
            throw WCError.invalidRequest("Invalid personal_sign parameters")

        default:
            throw WCError.invalidRequest("Unsupported personal sign method")
        }
    }

    private func extractTypedDataPayload(from request: WCSessionRequest) throws -> (address: String, rawData: Any) {
        guard let params = request.params as? [Any], params.count >= 2,
              let address = params[0] as? String else {
            throw WCError.requestTimeout
        }

        return (address, params[1])
    }

    private func extractTypedDataJSON(from request: WCSessionRequest) throws -> String {
        let payload = try extractTypedDataPayload(from: request)

        if let jsonStr = payload.rawData as? String {
            return jsonStr
        }

        if let jsonDict = payload.rawData as? [String: Any],
           let jsonData = try? JSONSerialization.data(withJSONObject: jsonDict),
           let str = String(data: jsonData, encoding: .utf8) {
            return str
        }

        throw WCError.invalidRequest("Invalid typed-data payload")
    }

    private func looksLikeEVMAddress(_ value: String) -> Bool {
        value.hasPrefix("0x") && value.count == 42
    }

    private func normalizeEVMAddress(_ value: String) -> String {
