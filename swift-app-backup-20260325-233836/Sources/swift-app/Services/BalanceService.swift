...
final class BalanceService: ObservableObject {
    ...
    private let maxBalanceRetryAttempts: Int = 5
    ...
    private func runBalanceFetchLoop(chainId: String, fetcher: @escaping () async throws -> String) async {
        ...
        while !Task.isCancelled {
            ...
            attempts += 1
            if attempts >= maxBalanceRetryAttempts {
                #if DEBUG
                print("⚠️ Balance fetch for \(chainId) gave up after \(attempts) attempts")
                #endif
                return
            }

            // Stop retrying non-transient errors (auth failures, etc.)
            if let tracker = balanceBackoff[chainId], tracker.isNonRetriable {
                #if DEBUG
                print("⚠️ Balance fetch for \(chainId) stopped: non-retriable error")
                #endif
                return
            }
    ...
    private func performBalanceFetch(chainId: String, fetcher: @escaping () async throws -> String) async -> Bool {
        ...
        do {
            ...
            // Mark auth errors as non-retriable (no point retrying without valid credentials)
            if let balanceError = error as? BalanceFetchError,
               case .invalidStatus(let code) = balanceError,
               code == 401 || code == 403 {
                tracker.markNonRetriable()
            }
