...
final class SyncEngine: ObservableObject {
    ...
    // Supported chain IDs
    private let supportedChains = ChainCapabilityRegistry.syncSupportedChainIDs
    ...
    func syncAll(walletId: String, addresses: [SyncAddress], force: Bool = false) async {
        ...
        let eligibleAddresses = addresses.filter { supportedChains.contains($0.chainId) }
        let totalChains = Double(eligibleAddresses.count)
        var completedChains = 0.0
        
        for address in eligibleAddresses {
