...
final class MultiProviderAPI: ObservableObject {
    ...
    /// Priority: Moralis (Trust Wallet) → CryptoCompare → CoinGecko
    func fetchPrices() async throws -> [String: Double] {
        ...
        // CoinCap API is defunct (DNS no longer resolves) — skipped
    ...
    func fetchSparkline(for chainId: String) async throws -> [Double] {
        // CoinCap API is defunct — skip directly to CryptoCompare
        
        // Try CryptoCompare first (reliable, generous limits)
