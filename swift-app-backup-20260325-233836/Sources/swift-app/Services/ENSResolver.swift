...
/// Supports ENS (.eth), selected Unstoppable Domains, and Solana SNS (.sol)
@MainActor
class ENSResolver: ObservableObject {
    ...
    enum DomainType {
        case ens       // .eth
        case ud        // .crypto, .wallet, .x, .bitcoin, .dao, .888, .zil, .blockchain
        case sns       // .sol
        case unknown
    ...
    func detectDomainType(_ domain: String) -> DomainType {
        ...
        let udTLDs = [".crypto", ".wallet", ".x", ".bitcoin", ".dao", ".888", ".zil", ".blockchain"]
        for tld in udTLDs {
