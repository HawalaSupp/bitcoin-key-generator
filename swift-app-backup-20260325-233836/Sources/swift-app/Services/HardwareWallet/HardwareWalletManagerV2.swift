...
public final class HardwareWalletManagerV2: ObservableObject, @unchecked Sendable {
    ...
    /// Find and connect to the first available device matching a saved account.
    public func connectToSavedAccount(
        _ account: HardwareWalletAccount,
        timeoutSeconds: Int = 30
    ) async throws -> DiscoveredDevice {
        if let device = discoveredDevices.first(where: { $0.deviceType == account.deviceType }) {
            if connectedWallets[device.id] == nil {
    ...
    public func getPublicKey(
        ...
        try await ensureSupports(chain: chain, on: wallet)
    ...
    public func signTypedData(
        ...
        try await ensureSupports(chain: .ethereum, on: wallet)
        
        return try await wallet.signTypedData(path: path, domainHash: domainHash, messageHash: messageHash)
    }

    /// Re-derive a saved account on the connected device before signing.
    public func verifySavedAccount(
        deviceId: String,
        account: HardwareWalletAccount,
        requestedChain: SupportedChain? = nil,
        verifyOnDevice: Bool = true
    ) async throws -> AddressResult {
        let chain = requestedChain ?? account.chain
        guard chain == account.chain else {
            throw HWError.accountChainMismatch(expected: account.chain.rawValue, requested: chain.rawValue)
        }
        ...
        let derivedAddress = try await getAddress(
            deviceId: deviceId,
            path: path,
            chain: chain,
            verify: verifyOnDevice
        )

        guard normalizedHardwareAddress(derivedAddress.address, for: chain) == normalizedHardwareAddress(account.address, for: chain) else {
            throw HWError.savedAccountMismatch(
                expectedAddress: account.address,
                actualAddress: derivedAddress.address,
                path: account.derivationPath
            )
        }

        if let publicKey = derivedAddress.publicKey, !account.publicKey.isEmpty {
            let actualPublicKey = publicKey.hexEncodedString()
            guard normalizedHardwarePublicKey(actualPublicKey) == normalizedHardwarePublicKey(account.publicKey) else {
                throw HWError.savedAccountMismatch(
                    expectedAddress: account.address,
                    actualAddress: derivedAddress.address,
                    path: account.derivationPath
    ...
    private func ensureSupports(chain: SupportedChain, on wallet: HardwareWallet) async throws {
        let supportedChains = await wallet.supportedChains
        guard supportedChains.contains(chain) else {
            throw HWError.unsupportedChain("\(wallet.deviceType.displayName) does not support \(chain.rawValue) signing in Hawala yet")
        }
    }

    private func normalizedHardwareAddress(_ address: String, for chain: SupportedChain) -> String {
        let trimmedAddress = address.trimmingCharacters(in: .whitespacesAndNewlines)

        switch chain {
        case .ethereum, .polygon, .arbitrum, .optimism, .avalanche, .bsc, .cosmos:
    ...
    private func normalizedHardwarePublicKey(_ publicKey: String) -> String {
        publicKey.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    }
