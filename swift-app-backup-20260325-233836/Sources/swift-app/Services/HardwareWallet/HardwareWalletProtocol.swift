...
public enum HWError: LocalizedError, Sendable {
    ...
    case accountChainMismatch(expected: String, requested: String)
    case savedAccountMismatch(expectedAddress: String, actualAddress: String, path: String)
    ...
    public var errorDescription: String? {
        ...
        case .accountChainMismatch(let expected, let requested):
            return "This hardware wallet account is saved for \(expected), but the current transaction requires \(requested)."
        case .savedAccountMismatch(let expectedAddress, let actualAddress, let path):
            return "Saved hardware wallet account mismatch for \(path). Expected \(expectedAddress), got \(actualAddress)."
