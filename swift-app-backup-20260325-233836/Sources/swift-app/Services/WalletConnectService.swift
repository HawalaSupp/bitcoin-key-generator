...
enum WCError: LocalizedError, Equatable {
    ...
    case hardwareWalletSigningUnavailable(String)
    case requestRequiresSendFlow
    ...
    var errorDescription: String? {
        ...
        case .invalidRequest(let details): return details
        case .accountNotAvailable(let address): return "This WalletConnect request targets \(address), which is not available in the active software wallet."
        case .hardwareWalletSigningUnavailable(let address): return "This WalletConnect request targets hardware wallet account \(address). Hardware-wallet message signing is not available in this flow yet."
        case .requestRequiresSendFlow: return "WalletConnect transaction requests must be reviewed and signed through Hawala's send flow."
        }
    }
}
