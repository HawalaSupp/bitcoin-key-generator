...
// ============================================================
// Edge Case #56: Spam Asset Label Filter
// ============================================================

@Suite("ROADMAP-19 #56: Spam Asset Label Filter")
struct SpamAssetLabelFilterTests {

    @Test("Detects airdrop spam")
    func airdropSpam() {
        #expect(EdgeCaseGuards.isLikelySpamAssetLabel(name: "FREE AIRDROP - Claim Now"))
    }

    @Test("Detects link-based spam")
    func linkSpam() {
        #expect(EdgeCaseGuards.isLikelySpamAssetLabel(name: "Prize", description: "Visit http://scam.xyz to claim"))
    }

    @Test("Normal asset label passes")
    func normalAssetLabel() {
        #expect(!EdgeCaseGuards.isLikelySpamAssetLabel(name: "Stablecoin Rewards Vault"))
    }

    @Test("Detects free mint spam")
    func freeMint() {
        #expect(EdgeCaseGuards.isLikelySpamAssetLabel(name: "Free Mint - Limited Edition"))
    }
}

// ============================================================
// Edge Case #57: Asset Metadata Fallback
// ============================================================

@Suite("ROADMAP-19 #57: Asset Metadata Fallback")
struct AssetMetadataFallbackTests {

    @Test("Generates fallback name with token ID")
    func fallbackName() {
        let name = EdgeCaseGuards.assetFallbackName(tokenId: "42", contractAddress: "0x1234567890abcdef1234567890abcdef12345678")
    ...
    func nonEmpty() {
        let name = EdgeCaseGuards.assetFallbackName(tokenId: "", contractAddress: "")
        #expect(!name.isEmpty)
    }
}
